# Contributing to the GitLab Monitor Home Assistant integration
...
- **Conventional Commits.** The version and the changelog are generated from the commit messages
  ...
  The release also syncs the version into `custom_components/gitlab_monitor/manifest.json`, which is
...
`hacs-ci.yml`, called with `component: gitlab_monitor` and `fuzz-script: fuzz/fuzz_parsers.py`. That
...
pytest tests/ -v --cov=custom_components/gitlab_monitor --cov-report=json:coverage.json
```
...
PYTHONPATH=. python fuzz/fuzz_parsers.py -max_total_time=30 -rss_limit_mb=1024
```

To try the integration against a real Home Assistant, copy `custom_components/gitlab_monitor` into your
